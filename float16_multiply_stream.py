...
st.set_page_config(page_title="Float16 Multiplier", layout="centered")
st.markdown("<h2 style='text-align:center;'>Float16 Multiplier</h2><hr>", unsafe_allow_html=True)
...
if a_str and b_str:
    ...
    if a_is_hex != b_is_hex:
        st.error("Both inputs must be the same format (either both decimal or both hex).")
    else:
        try:
            if a_is_hex:
                a = parse_hex16(a_str)
            ...
            result_bits = float16_to_bits(product)
            binary = f"{result_bits:016b}"
            s, e, m, formula = breakdown(result_bits)

            st.markdown("---")
            st.markdown(f"### ➡️ Result: `{float(product):.6g}`  &nbsp;&nbsp;|&nbsp;&nbsp; Hex: `0x{result_bits:04x}`")
