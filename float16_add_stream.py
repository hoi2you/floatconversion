...
st.set_page_config(page_title="Float16 Adder", layout="centered")
st.markdown("<h2 style='text-align:center;'>Float16 Adder</h2><hr>", unsafe_allow_html=True)
...
if a_str and b_str:
    ...
    if a_is_hex != b_is_hex:
        ...
        try:
            ...
            sum_result = np.float16(a + b)
            result_bits = float16_to_bits(sum_result)
            binary = f"{result_bits:016b}"
            s, e, m, formula = breakdown(result_bits)

            st.markdown("---")
            st.markdown(f"### ➕ Result: `{float(sum_result):.6g}`  &nbsp;&nbsp;|&nbsp;&nbsp; Hex: `0x{result_bits:04x}`")
